...
interface BudgetDashboardProps {
  ...
  lastAddedCategory: string | null; // To make recommendation titles more specific
  statusMessage: string; // To display status/feedback
}
  ...
  return (
    <div className="mt-10 space-y-10">
      ...
      <div className="max-w-xl mx-auto">
        {statusMessage && (
          <div className={`p-4 mb-6 rounded-md shadow ${recommendations.length > 0 ? 'bg-blue-50 border border-blue-300' : 'bg-slate-50 border border-slate-200'}`}>
            <p className={`text-center font-medium ${recommendations.length > 0 ? 'text-brand-blue' : 'text-slate-700'}`}>
              {statusMessage}
            </p>
          </div>
        )}
        ...
        {recommendations.length > 0 && (
          <div className="bg-white p-6 rounded-xl shadow-lg">
            <h2 className="text-2xl font-semibold text-brand-green mb-4 text-center">
              Smart Recommendations {lastAddedCategory && `for ${lastAddedCategory}`}
            </h2>
      ...
      {expenses.length === 0 && recommendations.length === 0 && !statusMessage.toLowerCase().includes("add your first expense") && (
